...
DECISION_VARS = CMAESVarSet(
    {
        ...
        "a_forward_rate_0": [0, 1 * SCALE],
        "a_turning_rate_0": [-1.5, 1.5],
        "a_forward_rate_1": [0, 1 * SCALE],
        "a_turning_rate_1": [-1.5, 1.5],
        "b_forward_rate_0": [0, 1 * SCALE],
        "b_turning_rate_0": [-1.5, 1.5],
        "b_forward_rate_1": [0, 1 * SCALE],
        "b_turning_rate_1": [-1.5, 1.5],
    }
)
...
def get_world_generator(n_agents, horizon, init=None, walls=True):
    ...
    def gene_to_world(genome, hash_val):
        ...
        heterogeneous_swarm.add_sub_populuation(a_agent, count=(n_agents - int(genome[0] * n_agents)))
        heterogeneous_swarm.add_sub_populuation(b_agent, count=int(genome[0] * n_agents))
...
if __name__ == "__main__":
    ...
    if args.sweep:
        cmaes.sweep_parameters([3, 3, 3, 3, 3, 3, 3, 3, 3])
    else:
        cmaes.minimize()
